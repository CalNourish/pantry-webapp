...
class Cart extends React.Component {
  ...
  addItem = (newItem, _quantity, defaultCart = false) => {
    ...
    if (isNaN(quantity)) {
      ...
      this.showError(
        `Quantity '${quantity}' is too high. Using default quantity 1.`,
        20000
      );
      quantity = 1;
    }
    ...
    if (items.has(barcode)) {
      /* if item already in table, just increment count */
      var itemData = items.get(barcode);
      if (itemData[1] + quantity > MAX_ITEM_QUANTITY) {
        this.showError(
          `New quantity '${itemData[1] + quantity}' is too high.`,
          20000
        );
        return;
      }
      ...
      /* otherwise, create new row in table */
  ...
  updateTotalCount = () => {
    let newCount = 0;
    this.state.items.forEach((val, _) => {
      newCount += val[1];
    });
  ...
  submitCart = async (e) => {
    ...
    fetch("/api/inventory/CheckoutItems", {
      method: "POST",
      body: reqbody,
  ...
  //   let item = newlimitedItems.find((item) => item.id === itemId);
  //   console.log(item)
  //   if (!item) {
  //     this.showError(
  //       "Data corruption: please try again or refresh page",
  //       20000
  //     );
  //     return;
  //   }
  ...
  render() {
    /* Hotkeys*/
    const quantityHotkey = "Q",
      barcodeHotkey = "B",
      searchHotkey = "F",
      searchSubmitHotkey = "F";
    ...
    document.onkeydown = (e) => {
      ...
      if (document.activeElement.classList.contains("quantity-input")) {
        ...
        if (e.key === "=" || e.key === "+") {
          // increment using plus key
          e.preventDefault();
          this.upItemQuantity(barcode);
        } else if (e.key === "-" || e.key === "_") {
          // decrement using minus key
          e.preventDefault();
          ...
          // navigate through quantities with arrow keys
          e.preventDefault();
      ...
      if (!this.state.isEditing) {
        if (
          (barcodeHotkey.toLowerCase() === e.key.toLowerCase() &&
          ...
          (quantityHotkey.toLowerCase() === e.key.toLowerCase() &&
            document.activeElement.id !== "barcode") ||
          e.key == "ArrowDown"
        ) {
          e.preventDefault();
          ...
          e.key.toLowerCase() === searchHotkey.toLowerCase() &&
          document.activeElement.id !== "barcode"
        ) {
          e.preventDefault();
      ...
      <Layout pageName='Checkout'>
        ...
        >
          <SearchModal
            ...
            addItemFunc={this.addItem}
            onCloseHandler={this.closeModal}
            submitHotkey={searchSubmitHotkey}
          />
        </Modal>
        ...
        <div className='flex flex-col h-full lg:flex-row'>
          ...
          <div className='flex-none lg:w-64'>
            <Sidebar className='sm:min-h-0 lg:min-h-screen'>
              ...
              <form
                id='checkout-item-form'
                onSubmit={(e) => this.itemFormSubmit(e)}
              >
                <h1 className='text-3xl font-medium mb-2'>Checkout Item</h1>
                <p className='mb-5 text-sm'>
                  Please enter the amount, then scan the item to add it to the
                  cart. Click "Check Out" to submit the cart.
                </p>
                <div className='form-group' id='barcode-and-quantity'>
                  <div className='col-xs-7 mb-4'>
                    <h1 className='text-2xl font-medium' autoFocus>
                      Barcode
                    </h1>
                    ...
                    <input
                      className='border rounded w-full py-2 px-3 text-gray-600 leading-tight'
                      id='barcode'
                      autoComplete='off'
                      autoFocus
                    ></input>
                ...
                >
                  Add Item{" "}
                  <span className='font-normal hidden lg:inline-block'>
                    (Enter)
                  </span>
                </button>
              </form>
          ...
          <div className='flex-none lg:w-150'>
            <Sidebar className='sm:min-h-0 lg:min-h-screen'>
              ...
              <div className='w-full'>
                <table style={{ width: "30rem" }}>
                  ...
                  <tbody className='relative'>
                          ...
                          <tr key={value.id}>
                            ...
                            <td className='pl-3.5'>
                              <input
                                className='w-full'
                                type='number'
                                defaultValue={value.noDependence}
                                onChange={(e) =>
                    ...
                    <tr className='relative'>
                      <td className='absolute' style={{ top: "-7%" }}>
                ...
                <div className='add-new-row flex justify-between w-full bg-black'>
                  ...
                  {/* <div className='input-name w-1'>
                    <input type='text' placeholder='Name:'></input>
                  </div>
                  <div className='input-no-dependence w-3/12'>
                    <input type='text' placeholder='No-dependence:'></input>
                  </div>
                  <div className='input-dependence w-3/12'>
                    <input type='text' placeholder='Dependence:'></input>
                  </div> */}
                </div>
              </div>
