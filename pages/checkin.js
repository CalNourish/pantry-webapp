...
class Checkin extends React.Component {
    ...
    if (this.state.lastScannedID == "N/A") {
      return (
        <div className="flex-grow text-left">
          Last Scanned ID: {this.state.lastScannedID}
        </div>
      );
    } else {
      return (
        <>
          <div className="flex-grow text-left">
            Last Scanned ID: {this.state.lastScannedID} at{" "}
            {this.state.lastScannedTime}.
          </div>
        </>
      );
    }
  ...
  showLastVisitInfo = () => {
    ...
    if (this.state.lastScannedID == "N/A") {
      ...
      if (numVisits == 0) {
        ...
        messageToReturn =
          "This visitor has already visited the pantry this week on  " +
          this.state.visitsLastWeek[0] +
          ".";
      } else if (numVisits == 2) {
        messageToReturn =
          "This visitor has already visited the pantry twice this week on  " +
          this.state.visitsLastWeek[0] +
          " and " +
          this.state.visitsLastWeek[1] +
          ".";
      } else if (numVisits > 2) {
        var messageToReturn =
          "This visitor has visited the pantry multiple times this week on ";
        for (var i = 0; i < numVisits; i++) {
          if (i == numVisits - 1) {
            messageToReturn =
              messageToReturn + this.state.visitsLastWeek[i] + ".";
          } else if (i == numVisits - 2) {
            messageToReturn =
              messageToReturn + this.state.visitsLastWeek[i] + ", and ";
          } else {
            messageToReturn =
              messageToReturn + this.state.visitsLastWeek[i] + ", ";
          }
  ...
  handleScanSubmit = (e) => {
    const calIDinBox = e.target.calID.value;

    if (e.target.calID.value.length == 0 || calIDinBox == null) {
    ...
    fetch("/api/admin/CheckIn", {
      method: "POST",
      body: JSON.stringify({ calID: calIDinBox }),
      ...
      .then((result) => {
        result
          ...
          .then((lastVisitedTimes) => {
            this.setState({
              lastScannedID: calIDinBox,
              visitsLastWeek: lastVisitedTimes,
              lastScannedTime: new Date().toLocaleTimeString(),
            });
            this.showSuccess("Sucessfully scanned ID: " + calIDinBox, 1000);
          })
          .catch((err) => {
            this.showError("Failed scanning ID: " + calIDinBox, 3000);
          });
      })
      .catch((err) => {
        this.showError("Failed scanning ID: " + calIDinBox, 300);
      })
      .finally(() => {
  ...
  render() {
    ...
    document.onkeydown = (e) => {
      if (
        !document.activeElement.classList.contains("calID") &&
        !isNaN(e.key)
      ) {
        e.preventDefault();
                ...
                <fieldset id="calIDFieldset" disabled={false}>
                  <div>
                    <div className="flex-grow">
                      Use scanner or manually enter Cal ID (put 1 if general
                      community member)
                    </div>
                    <input
                      className="calID border rounded w-2/3 py-2 px-3 text-gray-600 leading-tight"
                      placeholder="Cal ID"
                      id="calID"
                      autoComplete="off"
                      autoFocus
                  ...
                  <input
                    type="submit"
                    id="submitButton"
                    className="btn my-1 btn-pantry-blue uppercase tracking-wide text-xs font-semibold flex-grow disabled:bg-pantry-blue-400"
                    value="Submit (Enter)"
                  />
                </fieldset>
              </form>
